...
pub async fn update_guia(State(state): State<AppState>, auth: AuthUser, Path(id): Path<i32>, Json(request): Json<UpdateGuiaRequest>) -> Result<impl IntoResponse, ApplicationError> {
    request.validate().map_err(|e| ApplicationError::Validation(e.to_string()))?;
    let g = state.container.guia_repository.find_by_id(id).await?.ok_or_else(|| ApplicationError::NotFound(format!("Guía {} no encontrado", id)))?;
    let result = state.container.guia_repository.update(&request.apply_to(g, Some(auth.user.id))).await?;
    Ok(json_ok(GuiaResponse::from(result)))
}

#[instrument(skip(state, _auth))]
pub async fn delete_guia(State(state): State<AppState>, _auth: AuthUser, Path(id): Path<i32>) -> Result<impl IntoResponse, ApplicationError> {
    if !state.container.guia_repository.delete(id).await? { return Err(ApplicationError::NotFound(format!("Guía {} no encontrado", id))); }
    Ok(json_deleted())
}
