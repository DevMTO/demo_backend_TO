...
use super::common::{PaginatedResponse, PaginationInfo, json_ok, json_created, json_message, json_deleted};
...
#[instrument(skip(state, auth))]
pub async fn hard_delete_entrada(State(state): State<AppState>, auth: AuthUser, Path(id): Path<i32>) -> Result<impl IntoResponse, ApplicationError> {
