//! Handler para actualización de status de File
...
/// Actualiza el status de un file
...
pub async fn update_file_status(
    ...
    let current = state.container.file_repository
        .find_by_id(id)
        .await?
    ...
    Ok(json_ok(UpdateStatusResponse {
        success: true,
        mensaje: format!("Status de file actualizado de '{}' a '{}'", old_status, status.as_str()),
        old_status,
