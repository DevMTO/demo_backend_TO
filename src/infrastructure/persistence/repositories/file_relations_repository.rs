    ...
    async fn add_many(&self, id_file: i32, tours: Vec<FileTourInputData>, created_by: Option<i32>) -> Result<Vec<FileTourModel>, ApplicationError> {
        ...
        for data in tours {
            let new_record = NewFileTourModel {
                ...
                notas: data.notas.clone(),
                created_by,
                fecha_tour: data.fecha_tour,
                turno_tour: data.turno_tour.clone(),
                lugar_recojo: data.lugar_recojo.clone(),
                hora_recojo: data.hora_recojo,
                status: data.status.clone().unwrap_or_else(|| "pendiente".to_string()),
                geo_recojo: data.geo_recojo.clone(),
                nro_pasajeros: data.nro_pasajeros,
            };
    ...
    async fn find_by_file_with_tour(&self, id_file: i32) -> Result<Vec<FileTourWithTourModel>, ApplicationError> {
        ...
        let with_tour: Vec<FileTourWithTourModel> = results.into_iter().map(|(ft, (nombre, lugar_inicio, lugar_fin, duracion, tipo, is_active, geo_inicio, geo_fin, geo_ruta))| {
            FileTourWithTourModel {
                ...
                nro_pasajeros: ft.nro_pasajeros,
                tour_nombre: nombre,
    ...
    async fn update(&self, file_tour: &FileTourModel) -> Result<FileTourModel, ApplicationError> {
        ...
        let result: FileTourModel = diesel::update(file_tours::table.filter(file_tours::id.eq(file_tour.id)))
            .set((
                ...
                file_tours::notas.eq(&file_tour.notas),
